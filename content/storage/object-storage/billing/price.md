...
- **取回费用** 指的是对象存储将数据从低频存储取回到标准存储的费用。
- 内网下载数据，不收取费用。

## 免费额度

注册成功并完成认证的的对象存储用户，云平台为您提供一定额度的对象存储免费使用套餐。免费政策从用户创建 Bucket 之日算起，在未来 12 个月免费赠送:
...
> 需注意的是，雅加达区对象存储不在免费政策范围内。

## 欠费说明

若当月费用扣除后余额不足，则用户的所有 Bucket 将会处于 **暂停** 状态，此时用户无法使用对象存储的任何功能。等到用户充值足够的金额后，用户的所有 Bucket 将会自动恢复。
